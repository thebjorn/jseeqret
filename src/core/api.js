...
async function _ensureLoaded() {
  ...
  if (_vaultDir !== vaultDir) {
    _close()
    _vaultDir = vaultDir
  }

  if (!_key) {
    _key = fs.readFileSync(path.join(vaultDir, 'seeqret.key'), 'utf-8').trim()
  }
  ...
  if (!_db) {
    ...
    const buf = fs.readFileSync(path.join(vaultDir, 'seeqrets.db'))
 ...
 */
export function getSync(key, app = '*', env = '*') {
  if (!_db || !_key) {
 ...
 * Call this once at startup for fastest subsequent `getSync()` calls.
 */
